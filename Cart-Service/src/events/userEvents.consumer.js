...
const {
  upsertUserProjectionAndCart,
} = require("../services/cartProjection.service");
...
const parseUserCreatedEvent = (message) => {
  ...
  try {
    ...
    console.error("[cart-service] invalid JSON event", err);
    return null;
  }
  ...
  if (!Number.isInteger(userId) || userId <= 0) {
    console.error("[cart-service] invalid userId in USER_CREATED event", data);
    return null;
  }
...
const startUserEventsConsumer = async () => {
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      ...
      await upsertUserProjectionAndCart(userPayload);

      console.log(
        `[cart-service] USER_CREATED handled userId=${userPayload.userId} topic=${topic} partition=${partition} offset=${message.offset}`,
      );
    },
  });
};
