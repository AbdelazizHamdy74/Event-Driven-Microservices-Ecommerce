...
const {
  upsertPaymentUserProjection,
} = require("../services/paymentProjection.service");
...
const parseUserCreatedEvent = (message) => {
  ...
  try {
    ...
    console.error("[payment-service] invalid JSON event", err);
    return null;
  }
  ...
  if (!Number.isInteger(userId) || userId <= 0) {
    console.error("[payment-service] invalid userId in USER_CREATED event", data);
    return null;
  }
...
const startUserEventsConsumer = async () => {
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      ...
      await upsertPaymentUserProjection(userPayload);

      console.log(
        `[payment-service] USER_CREATED handled userId=${userPayload.userId} topic=${topic} partition=${partition} offset=${message.offset}`,
      );
    },
  });
};
